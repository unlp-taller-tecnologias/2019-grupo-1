class Evento(object):
    ...
    def create(cls, data, idUser, hoy):
        sql = """
            INSERT INTO evento (usuario_id,fecha,titulo,descripcion,fecha_evento,horario,links)
            VALUES (%s,%s,%s,%s,%s,%s,%s)
        """
        x = data['fecha_evento'].split('/')
        x = x[2]+'-'+x[1]+'-'+x[0]
        cursor = cls.db.cursor()
        cursor.execute(sql,(idUser,hoy,data['titulo'],data['descripcion'],x,data['horario'],data['links']))
        cls.db.commit()
    ...
    def edite(cls, data, hoy):
        sql = """  UPDATE evento SET fecha=%s,titulo=%s, descripcion=%s, fecha_evento=%s, horario=%s, links=%s  WHERE id =%s """
        cursor = cls.db.cursor()
        cursor.execute(sql, (hoy,data['titulo'],data['descripcion'],data['fecha_evento'],data['horario'],data['links'],data['idEvento']))
        cls.db.commit()
        return True
